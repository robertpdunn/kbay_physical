...
dli_monthly <- light_dli %>%
  ...
  select(date, reef,                   # select and reorder columns
         dli_month_mean, 
         dli_month_min, 
         dli_month_max, 
         dli_month_var) 
...
light_dli %>%
  mutate(mean_7 = rolling_mean(dli)) %>%
  group_by(reef) %>%
  ggplot(aes(x = date)) +
  geom_line(alpha = .3, aes(y = dli, color = reef)) +
  geom_line(size = 1, aes(y = mean_7, color = reef)) +
  labs(title = "Daily Light", x = "Date", y = "DLI (mol m-2 d-1)")
```
...
rolling_mean <- rollify(dli, window = 5)
