...
Get aily light integral (dli). First, group by reef and day, then take the mean of the light each day and convert to mol/m^2/day (multiply by 0.0864). 

```{r}
light_dli <- light_all %>%
  group_by(reef, date) %>%
  summarize(dli = mean(`PAR_calibrated_umol/m^2/s`*0.0864, na.rm=T)) %>%
  ungroup()
